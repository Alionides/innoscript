...
RUN apt-get update && apt-get install -y unzip libpq-dev libcurl4-gnutls-dev nginx libonig-dev libzip-dev
...
RUN mkdir -p /var/www/storage/app/public/websites
...
# Fix files ownership.
RUN chown -R www-data:1000 public
